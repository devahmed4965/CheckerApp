...
from MapWindow import MapWindow  # New import for map window functionality
...
class ManagerDashboard(ctk.CTk):
    def __init__(self, logged_in_user):
        ...
        self.TEXT_COLOR = "white"  # Fixed: now using self.TEXT_COLOR
    ...
    def build_main_content(self):
        ...
        # Top area: title and new buttons for attendance, map and report
        ...
        # Continue with existing main content setup
        ...
        btn_frame_emp = ctk.CTkFrame(self.content_scroll_frame)
        btn_frame_emp.pack(pady=10)
        ...
        # إضافة قسم لإعدادات النطاق الزمني والجغرافي (للمدير)
        self.build_geo_time_settings()

    def build_geo_time_settings(self):
        """قسم إعدادات النطاق الزمني والجغرافي في لوحة المدير."""
        settings_frame = ctk.CTkFrame(self.content_scroll_frame, fg_color="#4a4a4a", corner_radius=8)
        settings_frame.pack(fill="x", padx=10, pady=10)
        settings_title = ctk.CTkLabel(
            settings_frame,
            text="إعدادات النطاق الزمني والجغرافي",
            font=self.font_cairo_24_bold,
            text_color=self.TEXT_COLOR
        )
        settings_title.pack(pady=10)
        # إعدادات النطاق الزمني
        time_frame = ctk.CTkFrame(settings_frame, fg_color="#5a5a5a", corner_radius=8)
        time_frame.pack(fill="x", padx=10, pady=5)
        lbl_start = ctk.CTkLabel(time_frame, text="وقت بدء العمل (مثال: 08:00):", font=self.font_cairo_16, text_color=self.TEXT_COLOR)
        lbl_start.pack(pady=5)
        self.entry_working_start = ctk.CTkEntry(time_frame, width=200, font=self.font_cairo_16)
        self.entry_working_start.pack(pady=5)
        lbl_end = ctk.CTkLabel(time_frame, text="وقت انتهاء العمل (مثال: 17:00):", font=self.font_cairo_16, text_color=self.TEXT_COLOR)
        lbl_end.pack(pady=5)
        self.entry_working_end = ctk.CTkEntry(time_frame, width=200, font=self.font_cairo_16)
        self.entry_working_end.pack(pady=5)
        # إعدادات الموقع الجغرافي
        geo_frame = ctk.CTkFrame(settings_frame, fg_color="#5a5a5a", corner_radius=8)
        geo_frame.pack(fill="x", padx=10, pady=5)
        lbl_lat = ctk.CTkLabel(geo_frame, text="خط العرض (Latitude):", font=self.font_cairo_16, text_color=self.TEXT_COLOR)
        lbl_lat.pack(pady=5)
        self.entry_geo_lat = ctk.CTkEntry(geo_frame, width=200, font=self.font_cairo_16)
        self.entry_geo_lat.pack(pady=5)
        lbl_long = ctk.CTkLabel(geo_frame, text="خط الطول (Longitude):", font=self.font_cairo_16, text_color=self.TEXT_COLOR)
        lbl_long.pack(pady=5)
        self.entry_geo_long = ctk.CTkEntry(geo_frame, width=200, font=self.font_cairo_16)
        self.entry_geo_long.pack(pady=5)
        lbl_radius = ctk.CTkLabel(geo_frame, text="نطاق الشركة (بالمتر):", font=self.font_cairo_16, text_color=self.TEXT_COLOR)
        lbl_radius.pack(pady=5)
        self.entry_geo_radius = ctk.CTkEntry(geo_frame, width=200, font=self.font_cairo_16)
        self.entry_geo_radius.pack(pady=5)
        # زر حفظ الإعدادات
        btn_save_settings = ctk.CTkButton(settings_frame, text="حفظ إعدادات النطاق", font=self.font_cairo_16, command=self.save_company_settings)
        btn_save_settings.pack(pady=10)

    def save_company_settings(self):
        working_start = self.entry_working_start.get().strip()
        working_end = self.entry_working_end.get().strip()
        geo_lat = self.entry_geo_lat.get().strip()
        geo_long = self.entry_geo_long.get().strip()
        geo_radius = self.entry_geo_radius.get().strip()

        if not working_start or not working_end or not geo_lat or not geo_long or not geo_radius:
            messagebox.showerror("خطأ", "يرجى ملء جميع الحقول في إعدادات النطاق الزمني والجغرافي.")
            return
        ...
        try:
            ...
            messagebox.showerror("خطأ", "يجب أن تكون الإحداثيات والنطاق أرقاماً.")
            return

        session = SessionLocal()
        try:
            company = session.query(Company).filter_by(id=self.logged_in_user.company_id).first()
            if company:
                company.working_start = working_start
                company.working_end = working_end
                company.geo_latitude = geo_lat
                company.geo_longitude = geo_long
                company.geo_radius = geo_radius
                session.commit()
                messagebox.showinfo("نجاح", "تم تحديث إعدادات النطاق الزمني والجغرافي بنجاح.")
            else:
    ...
    # ----------------------------
    # New Functionality: Attendance, Map, and Report Buttons
    # ----------------------------
    def open_attendance(self):
        # فتح نافذة الحضور والانصراف
        AttendanceWindow(self)

    def open_map(self):
        # على سبيل المثال، يمكنك جلب إعدادات الشركة من قاعدة البيانات؛ هنا مثال بقيم ثابتة:
        company_lat = 24.7136   # استبدل بالإحداثيات الحقيقية
        company_lng = 46.6753
        company_radius = 500    # بالمتر
        MapWindow(self, company_lat, company_lng, company_radius)

    def open_report(self):
        messagebox.showinfo("تقارير", "ميزة عرض التقارير قيد التطوير")
    ...
    def check_shipment(self):
        ...
        try:
            if found_ship:
                ...
                from models import UnmatchedShipment
    ...
    # --------------------
    # New Functionality: Operation Tasks (for employees)
    # --------------------
